...
// 無効化テスト
window.postMessage(
  { source: "mr-wplace-front-tile-layer-update", enabled: false },
  "*",
);
```
...
2. ✅ **Refresh パフォーマンス改善** — State version を URL に含める方式を実装（完了）
3. ✅ **UI 実装** — popup menu に toggle ボタン追加（完了）
4. ✅ **Storage 永続化** — 設定を保存してページリロード時に復元（完了）

## 最新の実装（2026-02-11 更新）

### ✅ パフォーマンス改善完了

**State Version URL方式を実装:**
- `state-version.ts`: 状態バージョン管理
- タイルURLに`?v={stateVersion}`を付与してキャッシュバスティング
...
**Front-tile-layerでも正常動作:**
- `tile-overlay-renderer.ts`の実装が既に対応済み
