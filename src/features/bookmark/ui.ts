import { Bookmark, Tag, FavoriteLocation } from "./types";
...
export const createBookmarkModal = (): ModalElements => {
  ...
  modalElements.container.innerHTML = t`
    ...
    <!-- Bottom Tabs -->
    <div id="wps-bottom-tabs" style="flex-shrink: 0; display: flex; border-top: 1px solid oklch(var(--bc) / 0.1); margin-top: 0.5rem;">
      <button id="wps-tab-bookmark" class="btn btn-ghost flex-1" style="border-radius: 0; border-bottom: 2px solid oklch(var(--p)); color: oklch(var(--p)); font-weight: 600;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4">
          <path d="M200-120v-640q0-33 23.5-56.5T280-840h400q33 0 56.5 23.5T760-760v640L480-240 200-120Z"/>
        </svg>
        ${"bookmark"}
      </button>
      <button id="wps-tab-official-fav" class="btn btn-ghost flex-1" style="border-radius: 0; border-bottom: 2px solid transparent; color: oklch(var(--bc) / 0.5);">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4">
...
export const renderFavoriteLocations = (
  ...
  grid.innerHTML = locations
    .map((loc) => {
      const title = loc.name || `(${loc.latitude.toFixed(3)}, ${loc.longitude.toFixed(3)})`;
      const cardConfig: CardConfig = {
        id: `fav-${loc.id}`,
        title,
        subtitle: loc.name ? `${loc.latitude.toFixed(3)}, ${loc.longitude.toFixed(3)}` : undefined,
        padding: "11px 14px 10px 10px",
        onClick: true,
        data: {
