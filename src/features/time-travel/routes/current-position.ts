...
export class CurrentPositionRoute {
  ...
  private setupEvents(container: HTMLElement, router: TimeTravelRouter): void {
    // 保存ボタン
    container.querySelector("#wps-save-current-snapshot-btn")?.addEventListener("click", async () => {
      await this.saveCurrentSnapshot(container);
    });

    // 全タイル参照ボタン
    container.querySelector("#wps-browse-all-tiles-btn")?.addEventListener("click", () => {
      router.navigate("tile-list");
    });

    // スナップショット一覧のイベント委譲（既存コード流用）
    container.querySelector("#wps-current-snapshots-list")?.addEventListener("click", async (e) => {
  ...
  private async loadCurrentSnapshots(container: HTMLElement): Promise<void> {
    ...
    if (!position) {
      const listContainer = container.querySelector("#wps-current-snapshots-list");
      if (listContainer) {
    ...
    const listContainer = container.querySelector("#wps-current-snapshots-list");
    if (listContainer) {
      listContainer.innerHTML = snapshots.length === 0 
  ...
  private async deleteSnapshot(fullKey: string, container: HTMLElement): Promise<void> {
