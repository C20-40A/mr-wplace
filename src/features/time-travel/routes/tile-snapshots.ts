...
export class TileSnapshotsRoute {
  render(container: HTMLElement, router: TimeTravelRouter): void {
    ...
    container.innerHTML = t`
      <div class="mb-4">
        <div class="text-sm text-gray-600">タイル(${selectedTile.tileX}, ${selectedTile.tileY})のスナップショット</div>
      </div>
  ...
  private setupEvents(container: HTMLElement, router: TimeTravelRouter): void {
    ...
    container.querySelector("#wps-tile-snapshots-list")?.addEventListener("click", async (e) => {
      ...
      if (deleteBtn?.dataset.snapshotKey) {
        await this.deleteSnapshot(deleteBtn.dataset.snapshotKey, container, router);
  ...
  private async loadTileSnapshots(container: HTMLElement, tileX: number, tileY: number): Promise<void> {
    try {
      const snapshots = await TimeTravelStorage.getSnapshotsForTile(tileX, tileY);
      ...
      if (listContainer) {
        if (snapshots.length === 0) {
          ...
          listContainer.innerHTML = snapshots.map(snapshot => this.renderSnapshotItem(snapshot)).join('');
        }
      }
    } catch (error) {
  ...
  private async deleteSnapshot(fullKey: string, container: HTMLElement, router: TimeTravelRouter): Promise<void> {
