...
export class TimeTravelStorage {
  ...
  static async getSnapshotsForTile(tileX: number, tileY: number): Promise<SnapshotInfo[]> {
    ...
    return index.snapshots
      .filter(snapshot => snapshot.tileX === tileX && snapshot.tileY === tileY)
  ...
  static async addSnapshotToIndex(snapshot: SnapshotInfo): Promise<void> {
    const indexResult = await chrome.storage.local.get([this.INDEX_KEY]);
    const index: SnapshotIndex = indexResult[this.INDEX_KEY] || { snapshots: [], lastUpdated: 0 };
  ...
  static async removeSnapshotFromIndex(fullKey: string): Promise<void> {
    ...
    const index: SnapshotIndex = indexResult[this.INDEX_KEY];
    index.snapshots = index.snapshots.filter(snapshot => snapshot.fullKey !== fullKey);
    index.lastUpdated = Date.now();
