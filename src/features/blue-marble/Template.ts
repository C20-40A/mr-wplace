...
/** Color palette entry for template */
interface PaletteEntry {
  count: number;
  enabled: boolean;
}

/** RGB color meta information */
interface RgbMeta {
  id: number | string;
  premium: boolean;
  name: string;
...
export default class Template {
   ...
   *
   * @returns Collection of template bitmaps & buffers organized by tile coordinates
   */
  async createTemplateTiles(): Promise<{
    ...
    try {
      ...
      if (!inspectCtx)
        throw new Error("Failed to get 2D context for pixel inspection");
    ...
    const templateTiles: Record<string, ImageBitmap> = {}; // Holds the template tiles
    const templateTilesBuffers: Record<string, string> = {}; // Holds the buffers of the template tiles
