...
export class ImportExportService {
  static async exportFavorites(): Promise<ExportResult> {
    try {
      ...
      if (favorites.length === 0) {
        return { success: false, message: "エクスポートするお気に入りがありません" };
      }
      ...
      return { success: true, message: `${favorites.length}件のお気に入りをエクスポートしました` };
    } catch (error) {
  ...
  static importFavorites(): Promise<ImportResult> {
    return new Promise((resolve) => {
      const fileInput = document.getElementById("wps-import-file") as HTMLInputElement;
      if (!fileInput) {
      ...
      fileInput.onchange = async (e) => {
        ...
        try {
          ...
          if (!confirm(`${importCount}件のお気に入りをインポートしますか？\n既存のデータは保持されます。`)) {
            resolve({ success: false, message: "インポートがキャンセルされました" });
            return;
          }

          const newFavorites = importData.favorites.filter((importFav: Favorite) => {
            return !currentFavorites.some((existing: Favorite) =>
          ...
          await FavoriteStorage.setValue(STORAGE_KEYS.favorites, JSON.stringify(mergedFavorites));
