[UI 改善]
◎A.ギャラリー追加時（画像 1 枚目）に座標入力欄の追加要望（優先度：中、追加工数は小～中程度を想定）
画像の名前に座標が組み込まれていない場合を想定。
画像を Import するときのユーザーの脳内は「この画像をどこに読み込むか」を考えているため、後から座標を記入する現在の UI は直感的ではない。
座標を入力する際に、ギャラリーに追加 → 追加した画像をクリック → 座標を入力 →🔁 更新ボタンをクリック の手続きが煩雑
...
◎C.座標入力をクリック指定可能に（優先度：低、追加工数中程度を想定）
Skrik Marble と同様に、座標をクリックで指定できればうれしいです。が、頻度は高くないので優先度は低めです。
