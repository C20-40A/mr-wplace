...
- 描画しきれていない残りのピクセルがどこにあるかわかりにくい。複雑な画像の場合
- 画像追加でスワイプしても下が表示されない android
- データセーバーの説明テキストほしい
- タイルが描画されないことがある。removePreparedOverlayImageByKey 　が not defined が原因
- 描画カウントおかしい
...
題名　アーカイブの描画がうまくいかない
足立区だけ画面が閉じられず後ろで更新中と出るだけでうまくいかない（使用してる画像は 11/10 のもの）

それとおま環なのでどうでもいいですが一部の場所がブルーマーブルと併用しているとマス目がかなり小さく別の場所に描画されます
