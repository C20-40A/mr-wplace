...
- 閉じるボタンを押しやすくする機能を追加しました
  - ポップアップからON/OFFが変更できます
- 座標と座標コピーボタンを追加
- 「Eralyon」へのリンクボタンを追加
- 友人追加ボタンを追加
- ユーザーのタグと表示に対応
...
- PC版で、大きく見やすくしました
- 色ごとのピクセル数を表示し、減色しやすくしました
  - 未使用の色を無効化できるボタンも追加
- 「透明化ツール」を追加しました
  - 透過したい場所をクリックすると、その色を透過できます
  - 境界を調整することで、縁取りも可能です
- 画像のパン/ズームを少し使いやすくしました

## バグ修正/調整

- モバイル画面で、ペイント画面を少し見やすくしました
- レイヤー項目に進捗バーを表示
- オフラインキャッシュ設定でゴミ箱アイコンがクソでかになる問題を修正
- 画像セレクタのスクロール挙動を改善
- アーカイブ一覧からタイル一覧を表示したときに、画像が表示されない問題の修正

## v2.0.0 以降の小さな更新まとめ（ざっくり）

- チュートリアル画面の追加・拡充
- ピクセルグリッド表示の追加・高速化
- タイル境界表示/スムーズなマップ移動の復活
- 背景色変更やマップ表示補助 UI の統合
- フォントの追加
...
- いくつかの端末で、起動時にブラウザが落ちる問題を修正
- 起動直後のオーバーヘッドを削減しました
- ギャラリーの画像(テンプレート)の保存形式を変えました。起動時に自動でデータ移行処理が実行されます
- 画像を初めからタイルごとに分割することで、画像更新時の余計な分割処理をなくしました
- 画像のサムネイルを作成・保存するようにして、画像の一覧表示をより速く表示できるようにしました
- 誰かがタイルを更新するまで(etag の ID が変わるまで)、キャッシュしたタイル画像を再利用することで、描画処理を削減しました
- 画像編集で、値に変更がないときに処理をスキップすることで余計な画像処理を削減しました

## バグの修正

- 画像編集で大きい画像で再度更新しようとすると UI か崩れる問題の修正
- 画像編集で透明ピクセルが取り込まれてしまう問題の修正
- Paint たまったときの通知閾値を任意の値に設定できなかったので修正
- ギャラリー画面で、画像を非表示したあと、画像追加できなくなる問題の修正
- ギャラリー画面で、画像を非表示したあと、統計が消える問題の修正
- Android 環境で、一部 UI がスクロールできない問題の修正

## 細かい機能

- カラーフィルターの順番を wplace のカラーパレットと一致させました
- カラーフィルターで表示されるピクセル数は、最寄りのテンプレートのみ対象にするように変更
- ポップアップの UI を変更
- ペイント画面で閉じるボタンを押したとき、警告を表示できるようにしました。ポップアップの「Confirm on Close Paint Moda」から有効にできます
- モバイル端末で、ペイントするとき右上に表示される「ロック」ボタンを押しやすくする設定を追加しました。ポップアップの「Large Lock Button (for Mobile)」から設定を変更できます
- 開発者モード(コナミコマンド)で使えるツールを更新・高速化しました
- ペイント画面に、カラーフィルター画面を開くボタンを設置しました

## 描画方法を４つ追加

次の 4 つの描画方法を追加しました。低速なので、残り数ピクセルが見当たらないとき、目立たせるときに利用してください

- 巨大赤十字: 通常の赤十字が伸びます(30px)
- 巨大赤十字(極太): 赤十字が伸びる+太くなります
- 巨大赤ダイヤ: ピクセル周辺が大きなひし形で表示されます
- 巨大赤リング: ピクセル周辺に大きな円が表示されます
